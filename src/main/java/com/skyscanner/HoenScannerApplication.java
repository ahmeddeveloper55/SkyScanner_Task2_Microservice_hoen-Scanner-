...
import com.skyscanner.controller.SearchResource;
...
public class HoenScannerApplication extends Application<HoenScannerConfiguration> {
    ...
    public void run(final HoenScannerConfiguration configuration, final Environment environment) throws IOException {
        ...
        List<SearchResult> carResult  = Arrays.asList(
                mapper.readValue(
                getClass().getClassLoader().getResource("rental_cars.json"),
                SearchResult[].class
        )
        );
        List<SearchResult> hotelsResult = Arrays.asList(
                mapper.readValue(
        ...
        searchResult.addAll(carResult);
        searchResult.addAll(hotelsResult);
        final SearchResource resourse = new SearchResource(searchResult);
